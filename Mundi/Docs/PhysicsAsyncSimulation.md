...
![비동기 물리 시뮬레이션 아키텍처](PhysBench/slide_architecture.png)

가로가 시간, 세로가 계층입니다. 이후 장들은 이 그림의 각 부분을 코드 수준에서 설명합니다.

| 계층 | 무엇 |
|---|---|
| **게임 스레드** | `UWorld::Tick` 이 프레임을 진행시킨다 |
| **`FPhysScene`** | 게임 스레드와 PhysX 사이의 경계. PhysX 타입이 엔진 헤더로 새지 않게 PIMPL 로 감쌌다 |
| **PhysX** | `PxScene` 과 worker 스레드. solver 가 여기서 돈다 |

읽는 순서는 맨 위 가로 흐름입니다.

1. **`PhysScene->Tick()`** — `simulate()` 를 부르고 즉시 반환합니다. 아래로 내려가는
   `simulate()` 화살표가 solver 를 깨우는 지점입니다.
2. **`Actor Tick · Lua · Collision`** — solver 가 도는 동안 게임 스레드가 처리합니다.
   그림에서 **이 박스와 solver 막대가 같은 시간대에 놓여 있는 것**이 이 구현의 전부입니다.
3. **`PhysScene->EndFrame()`** — `fetchResults(true)` 로 완료를 기다립니다. solver 에서
   올라오는 `시뮬레이션 완료` 화살표가 그 시점입니다.

가운데 계층의 박스 셋(`scene read lock`, `PendingCommands`, `속도 캐시`)은 2번 구간에서만
의미가 있습니다. 그때 게임 코드가 시뮬레이션 중인 씬을 건드리기 때문이고, 접근 종류마다
다르게 막습니다 — [4장](#4-겹치는-동안의-안전성).

맨 아래 브래킷이 겹침 구간입니다. 이 구조로 줄일 수 있는 시간의 상한은
**`min(solver, Actor Tick)`** 이고, 둘 중 짧은 쪽이 끝나면 더 겹칠 것이 없습니다.
측정에서도 실측이 이 상한 아래를 따라갑니다.

### 코드에서의 순서

`UWorld::Tick` ([World.cpp:280-350](../Source/Runtime/Engine/GameFramework/World.cpp#L280-L350))
과 그림이 이렇게 대응합니다.

| 그림 | 코드 |
|---|---|
| — | `PhysScene->StartFrame()` |
| `PhysScene->Tick()` | `PhysScene->Tick(PhysDeltaTime)` |
| `Actor Tick · Lua · Collision` | Actor 순회 → `LuaManager->Tick()` → `ProcessPendingKillActors()` → `CollisionManager->UpdateCollisions()` |
| `PhysScene->EndFrame()` | `PhysScene->EndFrame()` |

`StartFrame()` 은 그림에서 뺐습니다. 통계 리셋과 차량 raycast 만 하고 겹침 구조에
관여하지 않습니다.

### 계층 — 왜 `FPhysScene` 과 `FPhysSceneImpl` 로 나눴나

그림 가운데 줄의 `FPhysScene` 은 실제로는 두 클래스입니다.
...
그림의 `simulate()` 화살표는 **매 프레임 발생하지 않습니다.** 물리는 프레임 시간이 아니라
**고정 간격 1/60 초**로 전진하기 때문입니다
...
- **프레임이 짧으면** 1/60 이 안 모여 그 프레임은 물리 스텝을 건너뜁니다. 그림의 solver
  막대가 없는 프레임입니다.
...
보간해 컴포넌트에 씁니다. 그림의 `결과 확정 후 처리` 박스에 있는 `렌더 보간` 이 이것입니다.
...
**보간이 `결과 확정 후 처리` 안에 있는 이유**가 있습니다. 보간 대상 목록을
`PxScene::getActiveActors()` 로 얻는데, 이 함수는 시뮬레이션이 진행 중일 때 호출할 수
없습니다. 그래서 `FetchResults()` 안에서 캡처 → 보간 순으로 처리합니다
...
그림 가운데 계층의 박스 셋입니다. `Actor Tick` 이 도는 동안 PhysX 씬은 worker 스레드가
쓰고 있고, 이때 게임 코드가 씬을 건드리면 문제가 됩니다. **접근 종류마다 다르게 막습니다.**

### `scene read lock` — 직전 스텝의 값을 본다

그림에서 `Actor Tick` 에서 내려오는 `읽기` 화살표입니다.
...
transform 은 `EndFrame()` 의 보간이 다시 덮어쓰므로 렌더 결과에는 영향이 없습니다.
...
### `PendingCommands` — 쓰기는 미뤘다가 나중에 적용한다

그림에서 `쓰기` 화살표가 들어가고, `지연 적용` 화살표로 `결과 확정 후 처리` 로 빠져나가는
경로입니다.
...
큐에 담기는 명령은 7종입니다 — `AddActor` · `RemoveActor` · `SetActorGlobalPose` ·
`AddForceToActor` · `AddTorqueToActor` · `SetActorLinearVelocity` ·
`SetActorAngularVelocity`
([PhysSceneImpl.h:142-160](../Source/Runtime/Engine/Physics/PhysSceneImpl.h#L142-L160)).
`FBodyInstance` 의 모든 쓰기 경로를 씬을 거치도록 바꿔야 이 게이트가 걸립니다.
...
그림의 `결과 확정 후 처리` 박스에 있는 `Transform · Velocity 캡처` 입니다.

### 충돌 이벤트 — 추가 동기화가 필요 없다

`fetchResults()` 가 내부에서 `PxSimulationEventCallback` 을 부릅니다. 즉 이벤트는
게임 스레드가 `EndFrame()` 에 들어와 있을 때 전달되며, 별도 처리가 필요 없습니다
...
그림 맨 아래 `solver — worker threads` 의 스레드 수입니다.
...
| 커밋 | 내용 | 그림에서 |
|---|---|---|
| [`f358fac4`](https://github.com/nansu0425/GameTechLab-WEEK13-public/commit/f358fac4) | Fixed timestep 도입. 프레임 델타 대신 1/60 고정 간격으로 전진 | `simulate()` 화살표의 발생 조건 |
| [`3d2814f3`](https://github.com/nansu0425/GameTechLab-WEEK13-public/commit/3d2814f3) | 스텝 사이 렌더 보간. 직전·현재 transform 을 들고 `Lerp` | `결과 확정 후 처리` 의 `렌더 보간` |
| [`5292ec1c`](https://github.com/nansu0425/GameTechLab-WEEK13-public/commit/5292ec1c) | **비동기 전환.** `simulate()` 와 `fetchResults()` 분리 | `Tick()` 과 `EndFrame()` 의 분리 |
| [`0e1ad224`](https://github.com/nansu0425/GameTechLab-WEEK13-public/commit/0e1ad224) | **`EndFrame()` 을 Actor Tick 뒤로 이동.** 겹침이 실제로 생기는 지점 | `겹침 구간` 브래킷 |
| [`a22562e2`](https://github.com/nansu0425/GameTechLab-WEEK13-public/commit/a22562e2) | `PhysicsSceneLock` — scene read/write lock RAII 래퍼 | `scene read lock` |
| [`a5d81a75`](https://github.com/nansu0425/GameTechLab-WEEK13-public/commit/a5d81a75) | worker 스레드 수를 `Max(논리 코어 - 1, 1)` 로 | `solver — worker threads` |
| [`92ee6c58`](https://github.com/nansu0425/GameTechLab-WEEK13-public/commit/92ee6c58) | `stat physics` — 시뮬레이션·fetch 시간, 액터·이벤트 수 오버레이 | — |
| [`03e44793`](https://github.com/nansu0425/GameTechLab-WEEK13-public/commit/03e44793) | 시뮬레이션 중 쓰기 지연과 속도 읽기 캐싱 | `PendingCommands`, `속도 캐시` |
| [`e150ba2a`](https://github.com/nansu0425/GameTechLab-WEEK13-public/commit/e150ba2a) | 렌더 보간을 `FetchResults()` 이후로 이동 | `렌더 보간` 의 위치 |
...
그림은 [`Tools/PhysBench/make_slides.py`](../../Tools/PhysBench/make_slides.py) 가 생성합니다.
